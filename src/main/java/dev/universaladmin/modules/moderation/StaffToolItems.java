...
public final class StaffToolItems {
    ...
    private static final int RANDOM_TELEPORT_SLOT = 3;
    ...
    private static final int[] TOOL_SLOTS = {
        PLAYER_INSPECTOR_SLOT, FREEZE_TOOL_SLOT, INVENTORY_INSPECTOR_SLOT, RANDOM_TELEPORT_SLOT,
        VANISH_TOGGLE_SLOT, MODERATE_TOOL_SLOT, TELEPORT_TOOL_SLOT, EXIT_STAFF_MODE_SLOT
    };

    private record ToolSpec(int slot, Material material, String labelKey) {}

    private static final Map<Tool, ToolSpec> SPECS = Map.of(
            Tool.PLAYER_INSPECTOR, new ToolSpec(PLAYER_INSPECTOR_SLOT, Material.PLAYER_HEAD, "moderation.gui.staffmode.tool.player-inspector"),
            Tool.FREEZE_TOOL, new ToolSpec(FREEZE_TOOL_SLOT, Material.PACKED_ICE, "moderation.gui.staffmode.tool.freeze"),
            Tool.INVENTORY_INSPECTOR, new ToolSpec(INVENTORY_INSPECTOR_SLOT, Material.CHEST, "moderation.gui.staffmode.tool.inventory-inspector"),
            Tool.RANDOM_TELEPORT, new ToolSpec(RANDOM_TELEPORT_SLOT, Material.ENDER_PEARL, "moderation.gui.staffmode.tool.random-teleport"),
            Tool.VANISH_TOGGLE, new ToolSpec(VANISH_TOGGLE_SLOT, Material.GLASS, "moderation.gui.staffmode.tool.vanish"),
            Tool.MODERATE_TOOL, new ToolSpec(MODERATE_TOOL_SLOT, Material.ANVIL, "moderation.gui.staffmode.tool.moderate"),
            Tool.TELEPORT_TOOL, new ToolSpec(TELEPORT_TOOL_SLOT, Material.COMPASS, "moderation.gui.staffmode.tool.teleport"),
            Tool.EXIT_STAFF_MODE, new ToolSpec(EXIT_STAFF_MODE_SLOT, Material.BARRIER, "moderation.gui.staffmode.tool.exit"));
     ...
     * {@link #updateHeldTool}. {@code RANDOM_TELEPORT}/{@code VANISH_TOGGLE}/
     * {@code EXIT_STAFF_MODE} act on the staff member themself, not a target,
     * so they're excluded.
     */
    private static final Set<Tool> STATUS_AWARE_TOOLS = EnumSet.of(
            Tool.PLAYER_INSPECTOR, Tool.FREEZE_TOOL, Tool.INVENTORY_INSPECTOR, Tool.MODERATE_TOOL, Tool.TELEPORT_TOOL);
     ...
     * reset to its plain default look. Re-rendered every tick a target
     * exists rather than only on change, so e.g. freezing the very player
     * being looked at shows up within one tick without the staff member
     * needing to look away and back.
     */
    public void updateHeldTool(Player staff, Tool held, Player target, boolean frozen, boolean vanished) {
        for (Map.Entry<Tool, ToolSpec> entry : SPECS.entrySet()) {
            ...
            ToolSpec spec = entry.getValue();
            ItemStack item = (tool == held && target != null)
